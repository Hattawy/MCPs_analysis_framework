...
#include <vector>
#include "TGraph.h"
vector<float> vx, vy, ve;
double num[10];
void GraphErrors1(char* finput)
{
	...
	while(*isin>>num[0]>>num[1]>>num[2])
	{
		vx.push_back(num[0]);
		vy.push_back(num[1]);
		ve.push_back(num[2]);
		cout<<vx.at(vx.size()-1)<<"\t"<<vy.at(vx.size()-1)<<"\t"<<ve.at(vx.size()-1)<<endl;
	}
	TGraphErrors *gr = new TGraphErrors(vx.size(), &(vx[0]), &(vy[0]), 0, &(ve[0]));
	gr->SetTitle("");
	gr->Draw("alp");
