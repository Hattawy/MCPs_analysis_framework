...
//     IgnoreWaveformData
//
//  Demostrate how to correctly read the Infiniium Public
                     ...
                     FILE* outputFile)
{
   ...
   if (waveformData)
   {
      ...
      int i,j=0;
	  printf("Output %d waveform points\n", waveformHeader->Points);
	  fprintf(outputFile,"# event %d \n", 0);
      for (i = 0; i < waveformHeader->Points; ++i)
      {
         double time = ComputeTimeFromIndex(i, waveformHeader);
         //fprintf(outputFile, "%e, %f\n", time, waveformData[i]);
//         if ( i%isample == 0 ) fprintf(outputFile, "%e %f\n", time, waveformData[i]); // time+vol
		 if ( i%isample == 0 ) fprintf(outputFile, "%f\n", waveformData[i]); // vol
      }
//	  fprintf(outputFile,"\n");
//	  fprintf(outputFile,"\n");
      success = 1;
...
int OutputWaveform(FILE* inputFile, FILE* outputFile)
{
   ...
   if (ReadWaveformHeader(inputFile, &waveformHeader))
   {
	  ...
	  printf("waveformHeader.WaveformType = %d\n", waveformHeader.WaveformType);
...
int main(int argc, char** argv)
{
   ...
   printf("PB_UNKNOWN, PB_NORMAL, PB_PEAK_DETECT, PB_AVERAGE, PB_HORZ_HISTOGRAM, PB_VERT_HISTOGRAM, PB_LOGIC = %d %d %d %d %d %d %d\n",
   PB_UNKNOWN, PB_NORMAL, PB_PEAK_DETECT, PB_AVERAGE, PB_HORZ_HISTOGRAM, PB_VERT_HISTOGRAM, PB_LOGIC);
   ...
   if (inputFile)
   {
      ...
      {
         int w;
		 printf("In main: fileHeader.NumberOfWaveforms=%d\n",fileHeader.NumberOfWaveforms);
         //if ((argc - 2) < fileHeader.NumberOfWaveforms)
		 // forse processing of at least 100000 waveforms
         if ((   100000    ) < fileHeader.NumberOfWaveforms)
         {
            // Not enough output files were provided
            // Use stdout to summarize input file
            printf("failed: Infiniium Public Waveform File version %c.%c\n",
         ...
         {
            printf("succeeded: Infiniium Public Waveform File version %c.%c\n",
                   fileHeader.Version[0],
                   fileHeader.Version[1]);
            FILE* outputFile = fopen(argv[2], "w");
			//printf("In main: forse number of waveform to be 1000\n");
            //for (w = 0; w < 1000; ++w)
			printf("In main:  processing %d waveforms. \n", fileHeader.NumberOfWaveforms);
            for (w = 0; w < fileHeader.NumberOfWaveforms; ++w)
            {
               if (outputFile)
               {
