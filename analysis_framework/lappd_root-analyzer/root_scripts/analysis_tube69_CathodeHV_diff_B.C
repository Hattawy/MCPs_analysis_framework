...
void analysis_tube69_CathodeHV_diff_B::Loop()
{
   ...
   const int N_B_field = 6;
   const int N_runs = 16*N_B_field;
   float B_vall[N_B_field] = { 0.01, 0.125, 0.125, 0.25, 0.5, 0.75};
   ...
   for(int i=0; i<NN; i++){
      h_Gains[i]     = new TH1F(Form("h_Gains[%d]",i+3000),"", 250, 100, 100000000);   
      h_amp[i]       = new TH1F(Form("h_amp[%d]",i+3000),"", 250, 0, 150);      
      h_timing[i]    = new TH1F(Form("h_timing[%d]",i+3000),"", 250, 500,70000);    
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      int var_rnum;
      if(10999 <runNum && runNum< 11016) 
        var_rnum = runNum-11000;
      else if (11099 <runNum && runNum< 11116)
        var_rnum = runNum-11100 + 16;
      else if (11199 <runNum && runNum< 11216)
        var_rnum = runNum-11200 + 16*2;
      else if (11299 <runNum && runNum< 11316)
        var_rnum = runNum-11300 + 16*3;
      else if (11399 <runNum && runNum< 11416)
        var_rnum = runNum-11400 + 16*4;
      else if (11499 <runNum && runNum< 11516)
        var_rnum = runNum-11500 + 16*5;



      if (run_flag[var_rnum] == false ){
         cout<<runNum<<"      "<< var_rnum<<endl;
         run_flag[var_rnum] = true;
       }  
 
      if( fwav_time[1]>0.0  && fwav_time[2]>0.0 && fwav_amp[1] >0.1  && fwav_amp[2] >0.1)
        {
...
//   }  
 ...
 output_txtfile.close();
   ...
   for( int i=0; i<N_B_field; i++){
      for(int j=0; j<N_HV_sets; j++){
           ...
           else if(i== 2) { nnum = j+16*2; }        
           else if(i== 3) { nnum = j+16*3; }        
           else if(i== 4) { nnum = j+16*4; }
           else if(i== 5) { nnum = j+16*5; }        
 ...
 int Find_runNumber(const char* DIRNAME)
 {
    ...
    int  FNP = fullNameLength - 10;   // first number position
