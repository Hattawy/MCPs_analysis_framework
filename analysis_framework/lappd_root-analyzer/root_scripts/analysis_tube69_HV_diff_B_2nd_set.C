#define analysis_tube69_HV_diff_B_2nd_set_cxx
#include "analysis_tube69_HV_diff_B_2nd_set.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void analysis_tube69_HV_diff_B_2nd_set::Loop()
{
   ...
   output_txtfile.open("final_analysis_tube69_HV_diff_B_2nd_set_txt_output.txt");

   // output root file 
   TFile *output_rootfile = new  TFile("final_analysis_tube69_HV_diff_B_2nd_set_output.root", "Recreate");
 ...
 for( int i=0; i<N_HV_sets; i++){
    ...
    for(int j=0; j<N_runs; j++){
       if( reading_data[j][2] == HV[i]){
         ...
         if( (h_amp[nnum]->GetEntries()) > 200.0) {
 
            cout<<" nnum = "<< nnum<<endl;
            h_amp[j]->Fit("gaus");
            TF1 *f1 = h_amp[j]->GetFunction("gaus");
            HV_amp_val[i][cc]  = f1->GetParameter(1);
            
            h_Gains[j]->Fit("gaus");
            TF1 *f2 = h_Gains[j]->GetFunction("gaus");
            HV_gain_val[i][cc]  = f2->GetParameter(1);
          
            h_timing[j]->Fit("gaus");
            TF1 *f3 = h_timing[j]->GetFunction("gaus");
            HV_time_res_val[i][cc]  = f3->GetParameter(2);
          
            h_pos_res[j]->Fit("gaus");
            TF1 *f4 = h_pos_res[j]->GetFunction("gaus");
            HV_pos_res_val[i][cc]  = f4->GetParameter(2);
